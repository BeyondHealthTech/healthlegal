...
const stats = [
  ...
  { number: "41%", label: "設計選択で対象外にできる項目", desc: "サーバレス構成の採用により、物理サーバ・媒体・施設前提の項目が対象外になる" },
...
const sources = [
  {
    title: "対応しない、を先に設計する",
    body: "全416項目のうち172件（41%）は、サーバレスAWS構成という設計選択の帰結として対象外にできます。物理サーバを持たない・可搬媒体を使わない・端末にデータを残さない——「頑張って対応する」前に「対応が要らない形を選ぶ」のがこのパッケージの出発点です。",
  },
  {
    title: "検討済みの標準回答416項目",
    body: "実際に3省2ガイドライン対応を通したプロダクトの検討結果を、どのプロダクトでも使える形に一般化した標準回答集。CloudTrail・Cognito・WAF・GuardDutyなど、AWSの設定レベルまで具体化されており、実装指示書としてそのまま使えます。",
  },
  {
    title: "文書ひな形66ファイル",
...
const steps = [
  ...
  { step: "2", title: "構成提案 ＋ 差分検討", duration: "1〜2週間", body: "サーバレスAWS構成の設計提案とセットで、個別検討が必要な項目（50件前後）だけをレビューします。" },
    ...
    <>
      ...
      <main>
        ...
        <section className="py-24 px-6 bg-gradient-to-b from-primary-50 to-white">
          <div className="max-w-4xl mx-auto text-center">
            ...
            <p className="text-slate-600 text-lg leading-relaxed max-w-2xl mx-auto mb-10">
              実際に3省2ガイドライン対応を通したプロダクトの実物から作った、
              標準回答・運用文書・セキュリティ設計の一式。
              416項目すべての検討を、標準回答からの差分レビューに変えます。
            </p>
            <a
              href="#contact"
        ...
        <section className="py-24 px-6 bg-primary-950">
          <div className="max-w-4xl mx-auto text-center">
            <h2 className="text-2xl sm:text-3xl font-bold text-white mb-6">実測に基づくフレームワーク</h2>
            <p className="text-primary-200 leading-relaxed max-w-2xl mx-auto">
              本パッケージの元になったフレームワークでは、
              <strong className="text-white font-semibold">416項目すべての該当性判断・対応方針の記入を、
              担当1名＋レビュー1名の体制・営業日10日で完了</strong>した運用実績があります。
              その検討結果と文書一式を、どのプロダクトにも適用できる形に一般化したのが本パッケージです。
            </p>
          </div>
        </section>
        ...
        <section className="py-24 px-6 bg-white">
          <div className="max-w-4xl mx-auto">
            ...
            <p className="text-sm text-slate-500 text-center mt-8">
              ※ サーバレスAWS構成の採用が前提です（構成のご提案とセットでご支援します）。
              構成が異なる場合もヒアリングのうえ適用範囲をご提示します。
            </p>
          </div>
        </section>
